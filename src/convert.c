                                         ...
                                         unsigned height)
{
	...
	dst->datalen = dst->width * dst->height;
...
void zbar_image_buffer_downsize( unsigned char* src, int w1, int h1,  
                          unsigned char* dest)
{
	int w2, h2;
	int x2, y2;
	...
	for (y2 = 0; y2 < h2; y2++){
		...
		for (x2 = 0; x2 < w2; x2++){
			sum = psrc[0] + psrc[1] + psrc[w1] + psrc[w1 + 1];
			*pdest = sum >> 2;
			pdest += 1;
			psrc += 2;
		}
...
zbar_image_t* zbar_image_downsize(const zbar_image_t *src)
{
	zbar_image_t *dst;
	int w1;
	int w2, h2;
	...
	dst->width = w2;
	dst->height = h2;
	dst->data = malloc(h2 * w2);
	dst->datalen = h2 * w2;
	dst->cleanup = zbar_image_free_data;
	dst->format = src->format;

	pdest = (unsigned char*)dst->data;
	for (y2 = 0; y2 < h2; y2++){
		psrc = src->data + y2 * 2 * w1;
